...
if (!prompt) return conn.sendMessage(m.chat, { text: `📍  Debe de ingresar una descripción para generar la canción.` }, { quoted: m });

conn.sendMessage(m.chat, { text: `Generando la canción, esto puede demorar unos minutos...` }, { quoted: m });

// Generar la canción con la API Sonu
const song = await generateMusicWithSonu(prompt, customTags || 'pop, romántico');

if (!song) return conn.sendMessage(m.chat, { text: `📍  No se ha podido generar la canción, intente con otra petición.` }, { quoted: m })
