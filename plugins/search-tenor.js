...
if (!text) return conn.sendMessage(m.chat, { text: `Ingrese el comando y escriba lo que quiera para buscar un gif en específico.\n\n• Por ejemplo:\n*#${command}* XD` }, { quoted: m })
try {
...
if (!gifs || gifs.length < 2) return conn.sendMessage(m.chat, { text: `📍  No se han encontrado resultados en la búsqueda.` }, { quoted: m })
const maxItems = Math.min(gifs.length, 10)
const medias = gifs.slice(0, maxItems).map(gif => ({ type: 'video', data: { url: gif.mp4 } }))
await sendAlbumMessage(m.chat, medias, { caption: `·─┄ · ✦ *GIF - Search* ✦ ·\n\n⊹ ✎ *Busqueda:* ${text}\n⊹ ✎ *Resultados:* ${maxItems} gifs encontrados.\n⊹ ✎ *Plataforma:* Tenor`, quoted: m })
} catch (e) {
