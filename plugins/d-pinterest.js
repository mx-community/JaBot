...
if (!text) return conn.sendMessage(m.chat, { text: `Ingrese el comando y escriba lo que quiera buscar en *Pinterest*.\n\n• Por ejemplo:\n*#${command}* Paisajes.` }, { quoted: m })
try {
await conn.sendMessage(m.chat, { text: `Buscando imagenes, espere un momento...` }, { quoted: m })
if (text.includes("https://")) {
let i = await dl(args[0])
let isVideo = i.download.includes(".mp4")
await conn.sendMessage(m.chat, { video: { url: i.download }, caption: `✓  Video descargado.` }, { quoted: m })
} else {
const results = await pins(text)
if (!results.length) {
return conn.sendMessage(m.chat, { text: `📍  No se han encontrado resultados de la busqueda, intentelo de nuevo.` }, { quoted: m })
}
const medias = results.slice(0, 10).map(img => ({ type: 'image', data: { url: img.image_large_url } }))
await conn.sendSylphy(m.chat, medias, {
caption: null, quoted: m })
await conn.sendMessage(m.chat, { text: `> ⩽ *Pinterest : Images* ⩾\n✎ *Busqueda:* "${text}"\n✎ *Resultado:* ${medias.length} imagenes.` }, m)
}} catch (e) {
}}
