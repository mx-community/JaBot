...
let optionsXd = `〆T I K T O K:D L
...
> ${textbot}`;
if (!text) return conn.reply(m.chat, optionsXd, m);
...
if (asd[0].imgSrc.length === 0) {
return conn.reply(m.chat, `📌  No se ha podido acceder al enlace.\n- Si el error persiste, reporta el comando.`, m);
}
await m.react('⏰');
...
console.error(e);
conn.sendMessage(m.chat, { text: `📍  Al parecer no se pudo descargar las imagenes, tipo de error [ ${e} ].` }, { quoted: m });
await m.react('❌');
}}
} catch (error) {
await m.react('❌');
