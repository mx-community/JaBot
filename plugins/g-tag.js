...
if (!m.quoted && !text) return conn.sendMessage(m.chat, { text: `Ingrese el comando y responda a un mensaje para mencionar a todos.` }, { quoted: m })
let mentionedJid = await m.mentionedJid
let users = participants.map(u => conn.decodeJid(u.id))
let htextos = text ? text : (m.quoted && m.quoted.text) ? m.quoted.text : "Undefined."
